...
5. Harden auth: verified-email registration (signed, expiring token), password minimum 8 chars (checked against a breached-password list if a library is available), secure session cookies (HttpOnly, Secure, SameSite), rate limiting on login/register/forgot-password (5/min/IP + backoff), generic error messages that don't reveal whether an email exists. Additionally, per GAP_REPORT: REQUIRE JWT_SECRET at boot (fail fast if unset — delete the "dev-secret" fallback in lib/env.ts); migrate auth from localStorage JWT to the secure cookies; hash password-reset AND invite tokens at rest, and give invite tokens an expiry with regenerate; make resolveEventFromRequest return 404 on an invalid event instead of silently falling back to a default event; restrict public GET /event/slug/:slug to slugs only (stop accepting raw event CUIDs).
...
8. Fix Surveys tenancy while routing every endpoint through the helper (surveys currently ignore x-event-id and always hit the default event), and fix allAttendeeUserIds() so community/broadcast notification fan-out targets the event's participants, never all users in the database.
9. Freeze apps/mobile: strategy is PWA-only. Add a README note in apps/mobile stating it is out of scope and must not be extended; exclude it from build/CI pipelines. Do not delete it.
10. Tests: tenancy isolation (org A admin cannot read/write org B's event by ID manipulation — list, detail, update, delete), role enforcement, auth flows, invite-link revocation, notification fan-out scoping.
...
6. Announcements: organizer event-wide announcements in-app; optional email for important ones (rate-limited). Three additions from buyer-evaluation research: (a) SEGMENTED targeting — send to everyone, a role (speakers/attendees), a specific session's joined attendees, or an attendance mode (in-person vs virtual); (b) PREVIEW/TEST mode — organizer sends the announcement to themselves first, one click; (c) EMERGENCY BROADCAST — a clearly-marked safety channel (evacuation, cancellation, urgent venue change) that BYPASSES the notification budget and quiet hours, requires a typed confirmation ("EMERGENCY") to send, and is audit-logged. Safety trumps calm; everything else respects the budget.
...
4. QR check-in: per-attendee QR (confirmation email + in-app); mobile staff scanner page (camera) validating and recording check-ins; live count. Scanner works for the STAFF role. The scanner must work OFFLINE with auto-sync: it caches the attendee list on load, records check-ins locally when the connection drops, syncs when it returns, and shows sync status — venue Wi-Fi failure during check-in is the category's classic catastrophic failure (buyer guides literally tell organizers to test this).
5. Organizer analytics dashboard: registrations over time, check-in rate, session popularity (joins, attendance, feedback), directory opt-in rate, messages/meetings volume, Q&A/poll participation — INGESTING the existing engagement-points data rather than inventing a new metric. Add an ADOPTION RATE headline metric: % of invited/registered attendees who actually activated and used the app during the event (industry average is only 40–60%; showing organizers ours — and helping them beat it with a pre-event reminder tool — is a selling point). CSV export on every table. EventSeries view: year-over-year trends across editions (the renewal chart).
...
- **Market anchors:** event apps run $1,000–$5,000 (small) to $5,000–$20,000 (mid-size); ~$5/attendee is common. Whova is quote-only (a complaint); Sched is $2.50–$4/attendee with documented annual-increase complaints; Sched now also markets flat-rate unlimited-events annual plans and a CFP with committee review to the education segment — expect them to defend that flank.
- **Buyer-behavior benchmarks (from evaluation-guide research):** 67% of organizers have switched or plan to switch platforms (the market is fluid — comparison pages and easy migration matter); average attendee adoption is only 40–60% (report ours, help organizers beat it); buyers model total cost at 75%/125% of expected attendance (keep tier boundaries generous); the killer procurement question is "it's 8 AM, my event starts in an hour, and the app isn't loading — what happens?" (the S2 status page, degradation mode, and honest ToS answer are the response); "no download required" is a real adoption lever for 200–500-person events — one no-download event app reports 71% day-of usage, which validates the PWA-first strategy and should appear in marketing copy.
