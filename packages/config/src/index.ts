...
/**
 * Deploy-time overrides for every brand value that names a host.
 *
 * BRAND-R (docs/BRAND_R_RUNBOOK.md) flips the code before DNS, so which origin
 * these strings should point at is a property of the deploy, not of the source.
 * The property accesses are written out literally because that is the only form
 * Next inlines into the browser bundle — reading `process.env` through a
 * variable would leave the client with `undefined` while the server had the
 * value, and the two renders would disagree. Only NEXT_PUBLIC_ names are read
 * for the same reason: a server-only variable cannot reach the browser, and a
 * brand string that differs between the two is a hydration error.
 */
function brandEnv(): {
  domain?: string;
  webBaseUrl?: string;
  ...
  legacyWebDomains?: string;
  legacyApiOrigins?: string;
} {
  try {
    return {
      domain: process.env.NEXT_PUBLIC_BRAND_DOMAIN,
      webBaseUrl: process.env.NEXT_PUBLIC_WEB_BASE_URL,
      supportEmail: process.env.NEXT_PUBLIC_SUPPORT_EMAIL,
      statusPageUrl: process.env.NEXT_PUBLIC_STATUS_PAGE_URL,
      legacyWebDomains: process.env.NEXT_PUBLIC_LEGACY_WEB_DOMAINS,
      legacyApiOrigins: process.env.NEXT_PUBLIC_EXTRA_API_ORIGINS,
    };
  } catch {
    // No `process` at all (non-Node, non-bundled consumer) — defaults stand.
    return {};
  }
}

/** First non-empty trimmed value; undefined when every candidate is blank. */
...
/** Comma/whitespace-separated env list → deduped entries, empty when unset. */
...
const BRAND_ENV = brandEnv();

/** Public brand host — the one that appears in prose and in canonical URLs. */
const brandDomain = firstSet(BRAND_ENV.domain) ?? "readyhall.com";
const brandPrimaryUrl = (firstSet(BRAND_ENV.webBaseUrl) ?? `https://${brandDomain}`).replace(/\/$/, "");

export const brand = {
  productName: "Readyhall",
  domain: brandDomain,
   ...
   * The favicon/app-icon set under /icons is referenced by <head> and the
   * manifest by name; these two are the ones code composes URLs from.
   */
  assets: {
    logo: "/brand/logo-256.png",
    /** Regenerated by `npm --workspace apps/web run gen:og`. */
  ...
  /**
   * Short form of the tagline — the line that sits under the logo. The longer
   * category descriptor lives in `marketingSeo.categoryLine`; both are true,
   * this one is the one a human reads first.
   */
  shortTagline: "Conference schedules and speaker readiness, without the chase.",
  /**
   * Live Better Stack status page (uptime monitoring configured 2026-08-02).
   *
   * Still on the legacy host, and deliberately so: the subdomain is a Better
   * Stack dashboard setting, and this URL is printed as visible text on
   * /security and in the help Contact article. Pointing it at a name nobody has
   * renamed yet would publish a dead status link — worse than an old one. Rename
   * it in Better Stack, then set NEXT_PUBLIC_STATUS_PAGE_URL.
   */
  statusPageUrl: firstSet(BRAND_ENV.statusPageUrl) ?? "https://ukedl.betteruptime.com",
  /**
   * Third-party account handles, not rendered anywhere today. Same rule as the
   * status page: these rename in the providers' dashboards first.
   */
  social: {
    x: "https://x.com/ukedl",
...
/**
 * BRAND-R transition (docs/BRAND_R_RUNBOOK.md §Sequencing).
 *
 * ukedl.com 301-redirects to readyhall.com indefinitely and stays registered,
 * so for as long as a browser can still be sitting on the old origin — a
 * bookmark, an emailed link, a cached page mid-cutover — both origins have to
 * be allowed through CORS and named in connect-src. Retiring the old brand is
 * then a variable change (set the list empty) rather than a code change.
 */
export const brandTransition = {
  /** Apex hosts, without scheme; each expands to https + www in the allowlist. */
  legacyWebDomains: (() => {
    const configured = BRAND_ENV.legacyWebDomains;
    return configured === undefined ? ["ukedl.com"] : envList(configured);
  })(),
  /**
   * API origins to name in connect-src on top of NEXT_PUBLIC_API_URL: the host
   * the cutover moves to (runbook Phase 1b) and the one it moves from, so the
   * policy is correct on either side of the flip and during it. Full origins —
   * the API lives on a subdomain, so there is no www form.
   */
  extraApiOrigins: (() => {
    const configured = BRAND_ENV.legacyApiOrigins;
    return configured === undefined
      ? [`https://api.${brandDomain}`, "https://api.ukedl.com"]
      : envList(configured);
  })(),
} as const;

/** `https://host` + `https://www.host` for one apex domain. */
function webOriginsForDomain(domain: string): string[] {
...
/**
 * Every web origin the API should accept, deduped and in a stable order: the
 * configured origin first (with its www/apex sibling), then the brand domain,
 * then the transition domains. The brand domain is included whichever way
 * WEB_BASE_URL currently points, which is what makes the allowlist correct both
 * before and after the DNS cutover. `configuredWebBaseUrl` is WEB_BASE_URL — a
 * localhost value in dev is kept as-is, since localhost has no www form worth
 * inventing.
 */
export function webOriginAllowlist(
  ...
  const configured = configuredWebBaseUrl.trim().replace(/\/$/, "");
  if (configured) {
    origins.push(configured);
    try {
      const { protocol, hostname, port } = new URL(configured);
      const authority = (host: string) => `${protocol}//${host}${port ? `:${port}` : ""}`;
      if (hostname.startsWith("www.")) {
        origins.push(authority(hostname.replace(/^www\./, "")));
      } else if (hostname.includes(".")) {
        origins.push(authority(`www.${hostname}`));
      }
    } catch {
      // Unparseable value — the literal origin is still allowed, nothing added.
    }
  }
  origins.push(...webOriginsForDomain(brand.domain));
  for (const domain of brandTransition.legacyWebDomains) origins.push(...webOriginsForDomain(domain));
...
export const consoleTabCopy = {
  ...
  certificates:
    "Certificates is where you design the certificate and set who earns one — our built-in layout with your accent color and logo, or a finished design you upload from Canva or anywhere else, with each attendee's name placed on it. Eligibility can be a check-in, a minimum number of sessions joined, or a required-session list. Issuing itself happens on Recap once the event has ended.",
} as const;
