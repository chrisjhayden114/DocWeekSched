...
export type FeatureGuideEntry = {
  ...
  imageSrc?: string;
  /**
   * Alt text for whichever screenshot resolves for this key — the manual
   * `imageSrc` or the CI-captured auto shot. Describes the surface in
   * SCREENSHOT_MANIFEST, so it stays true when a recapture replaces the file.
   */
  imageAlt?: string;
};
...
export const FEATURE_GUIDE: Record<FeatureKey, FeatureGuideEntry> = {
  community: {
    ...
    imageAlt: "Community board with channel pills, a composer, and the mixed thread feed below",
    whatItDoes:
  ...
  community_meetups: {
    ...
    imageAlt: "Meet-ups feed showing a gathering's format, time, and invite scope",
    whatItDoes:
  ...
  community_moments: {
    ...
    imageAlt: "Moments feed with a grid of attendee photos and their captions",
    whatItDoes:
  ...
  community_local: {
    ...
    imageAlt: "Local tips feed with a recommendation carrying an Open in Google Maps link",
    whatItDoes:
  ...
  community_icebreakers: {
    ...
    imageAlt: "Break the ice channel with the people-discovery carousel above the feed",
    whatItDoes:
  ...
  community_general: {
    ...
    imageAlt: "General Community feed with an audience pill on a targeted post",
    whatItDoes:
  ...
  messaging_dms: {
    imageAlt: "Messages inbox beside an open one-to-one thread",
    whatItDoes:
  ...
  messaging_requests: {
    imageAlt: "Messages inbox with a Requests section under the main threads",
    whatItDoes:
  ...
  messaging_groups: {
    imageAlt: "Group chat thread with its member names listed in the header",
    whatItDoes:
  ...
  session_qa: {
    imageAlt: "Session Q&A card with upvoted questions and one thread marked answered",
    whatItDoes:
  ...
  session_likes: {
    imageAlt: "Agenda card wall with the like control and its count on a session row",
    whatItDoes:
  ...
  engagement_points: {
    ...
    imageAlt: "The engagement gem and point total from the app top bar, shown close up",
    whatItDoes:
    ...
    goodToKnow:
      "On by default, but Free plans do not grant the entitlement — Per-event and above do. There is no public ranked list unless Public leaderboard ships later. Turning the display off does not erase points.",
  },
  public_leaderboard: {
    whatItDoes:
  ...
  breakout_style: {
    imageAlt: "Pick-one breakout agenda, one session chosen per timeslot",
    whatItDoes:
  ...
  attendee_directory: {
    imageAlt: "Attendees tab listing the members who opted into the directory",
    whatItDoes:
  ...
  matchmaker: {
    imageAlt: "Meet tab suggestion cards with why-lines and a Draft intro action",
    whatItDoes:
  ...
  concierge: {
    ...
    imageAlt: "Event assistant panel with its starter question chips above the input",
    whatItDoes:
  ...
  venue_maps: {
    ...
    imageAlt: "Venue floor plan with room-linked pins standing on the labeled rooms",
    whatItDoes:
  ...
  waitlist_visibility: {
    imageAlt: "Session header for a full session, with the Full — waitlist chip highlighted",
    whatItDoes:
    ...
    goodToKnow:
      "On by default, but the plan entitlement is off on Free, Per-event, and Pro — only Enterprise includes it today. Waitlist seats still work when the flag is off. Turning it off does not delete waitlist rows.",
  },
  daily_digest: {
    imageAlt: "Notifications tab showing the morning digest rollup",
    whatItDoes:
    ...
    goodToKnow:
      "On by default, with a stored default of daily. Free and Per-event plans do not grant the entitlement; Pro and above do. Focused, PD day, and Talk showcase store interrupts-only. Turning it off skips the morning rollup; existing notifications stay.",
  },
  cfp: {
    imageSrc: "/feature-guide/cfp.png",
    imageAlt: "Public call for presentations form with name, title, and abstract filled in",
    whatItDoes:
  ...
  session_polls: {
    imageAlt: "Live polls card on a session page with results spread across the options",
    whatItDoes:
    ...
    goodToKnow:
      "On by default. Free plans do not include polls; Per-event and above do. Focused and Talk showcase turn them off. Turning it off hides the card; existing polls stay.",
  },
  session_feedback: {
    imageAlt: "Session feedback card with the 1–5 rating and its comment box",
    whatItDoes:
    ...
    goodToKnow:
      "On by default. Free plans do not include it; Per-event and above do. Most presets leave it on (including Focused and Talk showcase). Turning it off hides the card; submitted ratings stay.",
  },
  sponsors: {
    imageSrc: "/feature-guide/sponsors.png",
    imageAlt: "Attendee sponsor strip with logos grouped under Gold, Silver, and Community headings",
    whatItDoes:
    ...
    goodToKnow:
      "On by default. Free plans do not include sponsors; Per-event and above do. Focused and PD day turn the strip off. Turning it off hides logos; sponsor rows and leads stay.",
  },
  sponsor_outreach: {
    imageSrc: "/feature-guide/sponsor_outreach.png",
    imageAlt: "Organizer sponsor outreach pipeline with prospect rows in each status",
    whatItDoes:
  ...
  checkin: {
    ...
    imageAlt: "An attendee's personal check-in QR code, captioned Show this at the door",
    whatItDoes:
    ...
    goodToKnow:
      "On by default. Free plans do not include check-in; Per-event and above do. Turning it off hides the Profile code and scanner calls; existing check-in records stay.",
  },
  ops_agent: {
    imageAlt: "Ops Inbox with drafted cards waiting to be applied or dismissed",
    whatItDoes:
    ...
    goodToKnow:
      "On by default. Free and Per-event plans do not include it; Pro and above do. The Focused preset turns it off. Turning it off stops new detections; existing cards stay.",
  },
  recap_agent: {
    imageAlt: "Recap workspace with its report, feedback synthesis, and certificate sections",
    whatItDoes:
      "Post-event recap builds a report after the event end date: synthesized sections, thank-you email drafts, and a metrics export. It can also draft certificate work when Certificates is on. Generate is blocked until the event has ended. Free and Per-event orgs see an upgrade prompt instead.",
    experience:
    ...
    goodToKnow:
      "On by default. Pro and above include it. The Focused preset turns it off. Turning it off disables report generation; a recap you already built stays stored.",
  },
  certificates: {
    imageSrc: "/feature-guide/certificates.png",
    imageAlt: "An issued certificate PDF carrying the event's accent color",
    whatItDoes:
      "Certificates let you define templates and issue downloads to eligible attendees after the event ends. A template uses either the built-in layout, which we compose with the event accent color and logo, or your own finished design — you upload a PNG or JPG, a Canva export say, and we place each attendee's name on it. Eligibility can be any check-in, a minimum number of joined sessions, or a required-session list (joins, not door scans). Batch issue runs in the background, and issued certificates can be verified on a public route.",
    experience:
      "Organizers design templates on the Certificates tab and issue from Recap. Attendees download from a link after the event (there is no separate attendee Certificates tab). Eligible people get a file, not a live editor.",
    goodToKnow:
      "On by default, but Free plans do not include certificates; Per-event and above do. Focused turns them off; PD day and Academic leave them on. Turning it off blocks new downloads; issued certificates stay. On an uploaded design the attendee's name is the only field we overlay — dates, hours and signatures belong in the design itself.",
  },
  paid_attendance: {
    imageAlt: "Participants table with the Payment column across its statuses",
    whatItDoes:
